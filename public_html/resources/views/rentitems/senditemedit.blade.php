...
@section('breadcrumb')
    <li class="breadcrumb-item">
        <a href="{{ route('useradmin.send.history') }}">{{__('Send History') }}</a>
    </li>
@endsection
@section('content')
<div class="row">
    <div class="col-xl-12">
        <div class="card">
            <div class="card-header">
                <h5></h5>
                <h3>Send Rent Items Edit</h3>
            </div>
            <hr>
                <div class="card-body table-border-style">
                    <div class="col-xl-12">
                        <x-auth-validation-errors class="mb-4 text-danger" :errors="$errors" />
                        <form method="post" action="{{ route('useradmin.send.update', $rent->rent_id) }}" id="sendForm">
                            @csrf
                            @method('PUT')
                            <?php
                            $user = Auth::user();
                            ?>
                            <input type="hidden" name="rent_id" id="rent_id" value={{ $rent->rent_id}}>
                            <input type="hidden" name="selected_items" id="selected_items" value="{{ old('selected_items',  $rentitems) }}">
                            <div class="row">
                            ...
                            <div class="row mt-4">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="event_id" id="event_id" class="form-control select2 mt-1">
                                            ...
                                            <option value="{{ $event->eid }}"
                                                {{ old('event_id', $rent->event_id) == $event->eid ? 'selected' : '' }}>
                                                {{ $event->event_name }}
                                            </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select name="rent_package_id" id="rent_package_id" class="form-control select2 mt-1">
                                            <option value="">Select Package</option>
                                            {{-- Dynamically populate the package dropdown based on the selected event --}}
                                            @foreach ($rentItemPackages as $rentItemPackage)
                                                <option value="{{ $rentItemPackage->id }}" {{ old('rent_package_id', $rent->rent_item_package_id) == $rentItemPackage->id ? 'selected' : '' }}>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select name="order_id" id="order_id" class="form-control select2 mt-1">
                                            ...
                                            <option value="{{ $order->order_id }}" {{ old('order_id', $selectOrder->order_id ?? '') == $order->order_id ? 'selected' : '' }}>
                                                    {{ $order->order_id }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select name="employee_id" id="employee_id" class="form-control select2 mt-1">
                                            <option value="">Select Employee</option>
                                            @foreach ($employees as $employee)
                                            <option value="{{ $employee->emp_id }}"
                                                {{ old('employee_id', $rent->employee_id) == $employee->emp_id ? 'selected' : '' }}>
                                                {{ $employee->name }}
                                            </option>
                                            @endforeach
                                        </select>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="customer_name" id="customer_name" class="form-control" value="{{ old('customer_name', $rent->customer_name) }}" readonly>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                ...
                                <table class="table dataTable mt-4">
                                    ...
                                    <tbody>
                                        @foreach ($items as $item)
                                            <tr @if($item->in_stock == 0) style="background-color: #484846" @endif>
                                                <td>{{ $item->item_name }}</td>
                                                <td>{{ $item->item_type }}</td>
                                                @if($item->in_stock == 0)
                                                    <td></td>
                                                    <td>
                                                        <input type="checkbox" id="external-item-{{ $item->item_id }}" onchange="toggleExternalItemModal(this, {{ $item->item_id }}, '{{ $item->item_name }}')">
                                                        <label for="external-item-{{ $item->item_id }}">External Supplier</label>
                                                    </td>
                                                    <td></td>
                                                @else
                                                    <td class="col-md-2">
                                                ...
                                                @if($item->in_stock == 0)
                                                    <div class="modal fade" id="externalSupplierModal-{{ $item->item_id }}" tabindex="-1" aria-labelledby="externalSupplierModalLabel-{{ $item->item_id }}" aria-hidden="true">
                                                        <div class="modal-dialog">
                                                            <div class="modal-content">
                                                                <div class="modal-header">
                                                                    <h5 class="modal-title" id="externalSupplierModalLabel-{{ $item->item_id }}">Select External Suppliers for {{ $item->item_name }}</h5>
                                                                    ...
                                                                    <form id="externalSupplierForm-{{ $item->item_id }}">
                                                                        <div id="supplier-entries-{{ $item->item_id }}">
                                                                            <div class="supplier-entry mb-3">
                                                                                ...
                                                                                <input type="number" class="form-control supplier-price" placeholder="Supplier Price" >
                                                                                <label for="supplier-quantity" class="form-label mt-2">Supplier Quantity</label>
                                                                                <input type="number" class="form-control supplier-quantity" placeholder="Supplier Quantity" >
                                                                                <button type="button" class="btn btn-danger btn-sm mt-2" onclick="removeSupplierEntry(this)">Remove</button>
                                                                            </div>
                                                                        </div>
                                                                        <button type="button" class="btn btn-success btn-sm mt-3" onclick="addSupplierEntry({{ $item->item_id }})">Add Another Supplier</button>
                                                                    </form>
                                                                </div>
                                                                <div class="modal-footer">
                                                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                                                                    <button type="button" class="btn btn-primary" onclick="saveExternalSuppliers({{ $item->item_id }}, '{{ $item->item_name }}')">Save</button>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            var oldSelectedItems = @json($rentitems);
            if (oldSelectedItems) {
                selectedItems = @json($rentitems);
                updateSendTable();
            }
        });
        // Initialize Select2
        ...
        function saveExternalSuppliers(itemId, itemName) {
            ...
            supplierEntries.forEach(entry => {
                ...
                const existingItem = selectedItems.find(item => {
                    if (item.itemId === itemId || item.item_id === itemId) {
                    return item.supplierId === parseInt(supplierId) || (item.suppliers && item.suppliers.some(supplier => supplier.id === parseInt(supplierId)));
                    }
                    return false;
                });
            ...
            if (eventId) {
             ...
             $.ajax({
                 ...
                 success: function(response) {
                    ...
                    $('#rent_package_id').append('<option value="">Select a Rent Item Package</option>');
                    for (var i = 0; i < rentItemPackages.length; i++) {
                        var rentItemPackage = rentItemPackages[i];
                        var isSelected = '@json(old('rent_package_id'))' == rentItemPackage.id ? 'selected' : '';
                        $('#rent_package_id').append('<option value="' + rentItemPackage.id + '" ' + isSelected + '>' + rentItemPackage.name + '</option>');
                    }
                     ...
                     for (var i = 0; i < orders.length; i++) {
                         var order = orders[i];
                         var isSelected = '@json(old('order_id'))' == order.order_id ? 'selected' : '';
                         $('#order_id').append('<option value="' + order.order_id + '">' + order.order_id + '</option>');
                     }
                 },
                    ...
                    if ( data.in_stock >= quantity) {
                        ...
                        if (quantity > 0) {
                            ...
                            for (var i = 0; i < selectedItems.length; i++) {
                                if (selectedItems[i].item_id == itemId) {
       ...
       function removeItem(button) {
           ...
           var itemId = rowToRemove.find('input[name="item_id[]"]').val();
           var supplierId = rowToRemove.find('input[name="supplier_id[]"]').val();

           selectedItems = selectedItems.filter(item => {
               if (item.suppliers && item.suppliers.length > 0) {
                   item.suppliers = item.suppliers.filter(supplier => !(supplier.id == supplierId && item.item_id == itemId));
                   return item.suppliers.length > 0;
               } else {
            ...
            for (var i = 0; i < selectedItems.length; i++) {
                ...
                if (item.suppliers && item.suppliers.length > 0 ) {
                    for (var j = 0; j < item.suppliers.length; j++) {
                        var supplier = item.suppliers[j];
                        sendTableBody.append(
                            '<tr>' +
                                '<td>' + itemName + ' - ' + supplier.supplier_name + '</td>' +
                                '<td>' +
                                    '<input type="number" class="form-control quantity-input-2"required oninput="validateQuantity(this, \'quantityValidationMessage2-' + item.item_id + '-' + supplier.id + '\'); checkQuantity(this, ' + item.item_id + ', \'' + item.itemName + '\' , ' + supplier.id + ')" value="' + supplier.rent_quantity + '">' +
                                    '<span class="text-danger" id="quantityValidationMessage2-' + item.item_id + '-' + supplier.id + '"></span>' +
                                '</td>' +
                                '<td>' +
                    ...
                    sendTableBody.append(
                        '<tr>' +
                            '<td>' + itemName + '</td>' +
                            '<td>' +
                                '<input type="number" class="form-control quantity-input-2" required oninput="validateQuantity(this, \'quantityValidationMessage2-' + item.item_id + '\'); checkQuantity(this, ' + item.item_id + ', \'' + item.itemName + '\', null)" value="' + item.quantity + '">' +
                                '<span class="text-danger" id="quantityValidationMessage2-' + item.item_id + '"></span>' +
                            '</td>' +
                            '<td>' +
                                '<button type="button" class="btn btn-danger" onclick="removeItem(this)">Remove</button>' +
                            '</td>' +
        ...
        function enableSubmitButton() {
            if ($('#employee_id').val() != 0 && selectedItems.length > 0 && $('#customer_id').val() != 0) {
      ...
      $(document).ready(function() {
        ...
        function checkQuantity(input, itemId, itemName, supplierId) {
            ...
            // Check if the item has suppliers
            if (item && item.suppliers && item.suppliers.length > 0) {
                // Check if the item is already in the array
                for (var i = 0; i < item.suppliers.length; i++) {
                    if (item.suppliers[i].id == supplierId) {
                        var existingItem = parseInt(item.suppliers[i].rent_quantity);
                        item.suppliers[i].rent_quantity = quantity;
                        updateSendTable();
                        return;
                    }
                }
