        ...
        <div class="card">
            ...
            <div class="card-header pb-0">
                <button class="btn btn-sm btn-primary me-2" type="button"
                    data-url="{{ route('useradmin.strategies.events.create_team_name') }}" data-size="md"
                    data-ajax-popup="true" data-title="{{ __('Create Team Name') }}">
            ...
            <div class="row">
                <div class="col-xl-12">
                    <div class="card-header card-body table-border-style">
                        <div class="col-xl-12">
                            <form method="POST" action="{{ route('useradmin.strategies.events.team.store') }}"
                                id="teamForm" enctype="multipart/form-data" data-ajax="true">
                                @csrf
                                <div class="row">
                                ...
                                <div class="task-container border rounded p-2 p-md-3">
                                    <div class="row">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control select1" name="team_category_id" id="team_name"
                                                required>
                                    ...
                                    <div class="task-container border rounded p-2 p-md-3">
                                        <div class="row">
                                                ...
                                                <div class="form-group mb-3">
                                                    ...
                                                    <select class="form-control" name="member_role" id="member_role">
                                                        ...
                                                        <option value="manager"
                                                            {{ old('member_role') == 'manager' ? 'selected' : '' }}>
                                                            Manager Team</option>
                                                        <option value="artist"
                                                            {{ old('member_role') == 'artist' ? 'selected' : '' }}>
                                                            Artist Team</option>
                                                        <option value="sales"
                                                            {{ old('member_role') == 'sales' ? 'selected' : '' }}>Sales
                                                            Team</option>
                                                        <option value="planner"
                                                            {{ old('member_role') == 'planner' ? 'selected' : '' }}>
                                                            Planner Team</option>
                                                    </select>
                                                </div>
                                            </div>
                                            ...
                                            <div class="col-md-6">
                                                <div class="form-group mb-3">
                                                    ...
                                                    <select class="form-control select1" name="member_id"
                                                        id="member_name">
                                            ...
                                            <div class="col-md-6">
                                                <div class="form-group mb-3">
                                                    <label class="form-label"
                                                        for="mobile">{{ 'Phone Number*' }}</label>
                                                    <x-input id="mobile" class="form-control" type="text"
                                        ...
                                        <div class="d-flex justify-content-between align-items-center mb-3">
                                            <button type="button" class="btn btn-sm btn-primary add-member"
                                                id="addMemberBtn">
                                ...
                                <div class="d-flex mb-3 justify-content-end">
                                    <div class="d-grid">
                                        <button class="btn btn-primary btn-block mt-2 from-prevent-multiple-submits"
...
<script>
    ...
    $(document).ready(function() {
        $('#event_name').on('change', function() {
            ...
            $('#selectedEventName').text(selectedText !== 'Select Event' ? selectedText :
                'None');
        });
    ...
    $('#addteamBtn').on('click', function(e) {
        ...
        const isDuplicate = allTeams.some(team => team.eventId === eventId && team.teamId ===
            teamId);
        if (isDuplicate) {
            $('#errorMessage').text(
                `A team named "${teamName}" has already been added for the event "${eventName}".`
            ).show();
            return;
        }
    ...
    function save(formId) {
        ...
        fetch("{{ route('useradmin.strategies.events.team.store') }}", {
                method: "POST",
                headers: {
                ...
                body: JSON.stringify({
                    teams_data: JSON.stringify(allTeams)
                })
            })
            .then(response => {
                if (response.ok) {
