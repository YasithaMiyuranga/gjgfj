@extends('layouts.app')
@section('page-title', __('Rent Edit Package'))
@section('breadcrumb')
    <li class="breadcrumb-item">
        <a href="{{ route('useradmin.event_rent_packages.create') }}">{{__('Rent Edit Package') }}</a>
    </li>
@endsection
@section('content')
<div class="row">
    <div class="col-xl-12">
        <div class="card">
            <div class="card-header">
                <h5></h5>
                <h3>{{ ('Rent Edit Package') }}</h3>
            </div>
            <hr>
            <div class="card-body table-border-style">
                <div class="col-xl-12">
                    <x-auth-validation-errors class="mb-4 text-danger" :errors="$errors" />
                    <form method="post" action="{{ route('useradmin.event_rent_packages.update', $rentItemPackage->id) }}" id="EventRentForm">
                        @csrf
                        @method('PUT')
                        <?php
                        $user = Auth::user();
                        ?>
                        <input type="hidden" name="selected_items" id="selected_items" value="{{  old('selected_items' ,$rentItemPackage->rent_item_packages_items) }}">
                        <div class="row">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="event_id" id="event_id" class="form-control select2 mt-1">
                                        ...
                                        <option value="{{ $event->eid }}"
                                            {{ old('event_id') == $event->eid || $rentItemPackage->event_id == $event->eid ? 'selected' : '' }}>
                                            {{ $event->event_name }}
                                        </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                   ...
                                   <textarea id="description" class="form-control" name="description" rows="3" maxlength="255">{{ old('description',$rentItemPackage->description) }}</textarea>
                                </div>
                            </div>
                            {{-- <div class="col-md-6">
                                <div class="form-group">
                        ...
                        <div class="row">
                            <div class="col-md-4 mb-2">
                                <button type="submit" class="btn btn-primary from-prevent-multiple-submits mt-5" id="submitBtn"
...
<script>
    $(document).ready(function() {
       var oldSelectedItems = document.getElementById('selected_items').value;
        if (oldSelectedItems) {
        ...
        for (var i = 0; i < selectedItems.length; i++) {
            ...
            sendTableBody.append(
                '<tr>' +
                    '<td>' + item.itemName + '</td>' +
                    '<td>' +
                        '<input type="number" class="form-control quantity-input-2"  oninput="validateQuantity(this, \'quantityValidationMessage2-' + item.item_id + '\'); checkQuantity(this, ' + item.item_id + ', \'' + item.itemName + '\')" value="' + item.quantity + '">' +
