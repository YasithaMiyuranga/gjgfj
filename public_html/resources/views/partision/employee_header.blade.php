        ...
        <ul class="list-unstyled w-100 justify-content-between justify-content-lg-start">
            ...
            <li class="dropdown dash-h-item drp-company">
                <a class="dash-head-link dropdown-toggle arrow-none me-0" data-bs-toggle="dropdown" href="#"
                    ...
                    <span class="theme-avtar">
                        <img alt="#" style="width:30px;"
                            src="{{ !empty(Auth::guard('employee')->user()->profile_image) ? asset(Auth::guard('employee')->user()->profile_image) : asset('assets/images/profile/Avatar.png') }}"
                            class="header-avtar">
                    </span>
                    <span class="hide-mob ms-2">
                        @if(!Auth::guest())

                        {{ __('Hi, ') }}{{ !empty(Auth::guard('employee')->user()) ? Auth::guard('employee')->user()->name : '' }}!
                        @else
                        {{ __('Guest') }}
                ...
                @if(Auth::guard('employee')->check())
                    ...
                    <a href="{{ route('employee.emp.profile')}}" class="dropdown-item">
                        <i class="ti ti-user"></i>
                        <span>{{ __('Profile') }}</span>
                    </a>
                    <form method="POST" action="{{ route('emp.logout') }}" id="form_logout">
