...
@section('breadcrumb')
    <li class="breadcrumb-item">
        <a href="{{ route('useradmin.show_agreement_event') }}">{{('Agreements') }}</a>
    </li>
@endsection
@section('content')
<div class="row">
    <div class="col-xl-12">
        <div class="card">
            <div class="card-header">
                <h5></h5>
                <h3>{{ ('Event Agreement List') }}</h3>
            </div>
            <hr>
                <div class="card-header pb-0">
                    <div class="d-flex justify-content-between flex-row">
                        <a href="{{ route('useradmin.agreement.view_event') }}" class="btn btn-primary btn-sm">
                            ...
                            <tbody>
                                @foreach ($agreements as $agreement)
                                    <tr>
                                        ...
                                        <td>{{ $agreement->event->event_name ?? 'N/A' }}</td>
                                        <td>{{ $agreement->template->name ?? 'N/A' }}</td>
                                        <td>{{ $agreement->template->agreementCategory->name ?? 'N/A' }}</td>
                                        <td>
                                            <a href="{{ route('useradmin.agreement.edit_event', $agreement->id) }}">
...
<script>
    function downloadpdf( agreementId) {
        ...
        xhr.open('GET', `/useradmin/agreements/generate/event/pdf/download/${agreementId}`, true);
        ...
        xhr.onload = function() {
            if (xhr.status === 200) {
                ...
                window.location.href = "{{ route('useradmin.show_agreement_event') }}";
            } else {
