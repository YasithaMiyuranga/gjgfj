...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active">{{ __('Create Task Templates') }}</li>
    </li>
@endsection
@section('content')
    <style>
    ...
    <div class="row">
        <div class="col-xl-12">
            <div class="card">
                <form method="POST" id="task-template-form" action="{{ route('useradmin.task_templates.store') }}">
                    ...
                    <div class="card-body table-border-style p-15">
                        ...
                        <div class="mb-4 d-flex justify-content-between align-items-center">
                            {{-- <button disabled type="button" id="add-but" class="btn btn-success btn-sm add-task">
...
<div class="modal fade" id="addTaskModal${taskIndex}" tabindex="-1" aria-labelledby="addTaskModalLabel${taskIndex}" aria-hidden="true">
        ...
        function addTask(name1) {
            const name = $(`#modal-task-name${name1}`);
            const duration = $(`#modal-task-duration${name1}`);
            const priority = $(`#modal-task-priority${name1}`);

            if (name[0].value.trim() === "") {
                name[0].setCustomValidity("Please enter a valid name");
                name[0].reportValidity();
                return;
            } else {
                name[0].setCustomValidity(""); // clear previous errors
            }

            if (duration[0].value.trim() === "") {
                duration[0].setCustomValidity("Please enter a valid duration");
            ...
            if (priority[0].value.trim() === "") {
                priority[0].setCustomValidity("Please select a valid priority");
                ...
                const html = createTaskCard(taskIndex, name.val(), duration.val(), priority.val(), name1);
                $(`.tasks-column[data-column="${name1}"]`).append(html);
                taskIndex++;
        ...
        //         name[0].reportValidity();
        //     } else if (!duration[0].checkValidity()) {
        //         duration[0].reportValidity();
        //     } else if (!priority[0].checkValidity()) {
        //         priority[0].reportValidity();
        //     } else {
...
<div class="modal fade" id="addTaskModal${name}" tabindex="-1" aria-labelledby="addTaskModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="addTaskModalLabel">Add New Task</h5>
                    <button type="button" onclick="hideAddTaskModal('${name}')" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="form-group mb-3">
                        <label for="modal-task-name">Task Name*</label>
                        <input type="text" name="modal-task-name" id="modal-task-name${name}" class="form-control">
                    </div>
                    <div class="form-group mb-3">
                        <label for="modal-task-duration">Duration*</label>
                        <input type="text" name="modal-task-duration" id="modal-task-duration${name}" class="form-control">
                    </div>
                    <div class="form-group mb-3">
                        <label for="modal-task-priority">Priority*</label>
                        <select name="modal-task-priority" id="modal-task-priority${name}" class="form-control">
                            <option value="" selected>Select Priority</option>
                            <option value="Low">Low</option>
                            <option value="Medium">Medium</option>
                            <option value="High">High</option>
                ...
                <div class="modal-footer">
                    <button type="button" onclick="hideAddTaskModal('${name}')" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                    <button type="button" class="btn btn-success" id="confirm-add-task" onclick="addTask('${name}')">Add Task</button>
                </div>
            </div>
        </div>
    </div>
                    ...
                    <div class="border rounded p-2 mb-3 border-white custom-column-container" data-column="${name}">
                        <div class="column-header text-start mb-3">
                            <div class="column-title-wrapper position-relative mb-1">
                                ...
                                <div style="display:flex;justify-content: end;gap:5px">
                                    <button id="task-but${name}" type="button" onclick="showAddTaskModal('${name}')" class="btn btn-sm btn-outline-success top-0 end-0 " title="Add Task"><i class="fas fa-plus"></i></button>  
        ...
        function showAddTaskModal(name) {
        ...
        function hideAddTaskModal(name) {
            const modalElem = document.getElementById(`addTaskModal${name}`);
